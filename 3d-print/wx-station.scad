...
// Select the part(s) to render. Note that the ./render.sh bash script generates ALL parts.
Part = -8;				// [-1:Render all (for preview),1:Housing -- Bottom Plate,2:Main Housing,3:Part 3,4:Bottom Bearing -- Inner,5:Bottom Bearing -- Outer,6:Anemometer Cups,7:Top Bearing -- Inner,8:Windvane Tail,9:Part 9,10:Housing -- Top,11:Part 11,12:Part 12,13:Rain Cylinder,14:Stand,15:PoE Box,16:Part 16,17:Alternative Inner,18:Part 18]

// The overall resolution with which to render polygons (default: 100)
resolution = 100;		// [25:250]

// Smooth tuning bearing -- ± inner diameter (in µm)
BearingInTuning = +20;	// [-50:50]

// The diameter of the mounting tube/pole (in mm)
MountDia = 42.25;

// The polygon resolution of the mount tube; use 4 for square
MountDiaFn = 100;	// [4:Square (4),25:Rough (25),100:Smooth (100),250:Ultra-smooth (250)]

// Render a split/cut (half) view
CUT = false;

// Enhance the strength of Part 6
...
difference() {
	...
	// Cut output into cross section if we are rendering the whole output
	if ((CUT == true) && (Part < 0)) {
//		rotate([0,0,-90])
...
//		color([0,1,0])  translate([0, 0, 8.5+1]) rotate([0,90,0]) import("61802.stl", convexity = 5);
	...
	// POE     // 15
	if (Part == 15) {
		translate([0, -8, 17.5]) rotate([180, 0, 0]) POEboxTOP();
//		color([0,0,1]) rotate([180,0,90]) translate([4,1,-14.5]) import("poe-injector-05.stl", convexity = 5);
	...
	// Stand        // 14
	if (Part == 14 || Part < 0) {
		color([0.85, 0.00, 0.00, 0.85]) translate([0, -RODshift, -25]) Stand(20);
	}
	...
	if (Part == 13) {
		...
		color([0.00, 0.35, 0.85, 0.35]) RainCylinder();
	}
    ...
    difference() {
        ...
        difference() {
            hull() {
                ...
                translate([40,-6, 177.5]) cylinder(h=8, d1=20-6, d2=14+7, center=false, $fn=200);
                translate([-40,-6, 177.5]) cylinder(h=8, d1=20-6, d2=14+7, center=false, $fn=200);
            }
            CrosSupport();
        }
...
//		EyeBase0();
		$fn=150;
		color([0.85, 0.85, 0.00, 0.35]) EyeBase1();
	}
...
module din() {
	...
	difference() {
        ...
        translate([9+expand,2.9,width/2+z2shift/2]) rotate([-90,0,0]) {cylinder(h=3, d1=screwSmall, d2=3.2, center=false);}
